<template>
  <div :class="[fileIn?'expand':'h-9', 'file_list', 'flex-grow-0', 'relative', 'transition-all', 'duration-300', 'ease-out', 'flex', 'flex-col']" @dragover="allowDrop" @drop="fileDrop">
    ...
    <div class="tips absolute z-20" v-show="fileList.length === 0">
      kjlkjlk
    </div>
  ...
  methods: {
    fileDrop(event){
        ...
        event.dataTransfer.files.forEach(f => {
            ...
            if(!tempList.find(Element => Element.path === f.path)&&this.acceptType.includes(rawList[rawList.length - 1].toLowerCase())){
              tempList.push({
                  name: f.name,
                  path: f.path,
                  stats: 'ready',
